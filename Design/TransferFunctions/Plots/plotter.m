...
lt_tf_err = []; v_pid_tf_err = []; i_pid_tf_err = []; d_pid_tf_err = [];
% lt_delay_err = []; v_pid_delay_err = []; i_pid_delay_err = []; d_pid_delay_err = [];
% lt_ph_err = []; v_pid_ph_err = []; i_pid_ph_err = []; d_pid_ph_err = [];
...
for i = sweep_start:sweep_step:sweep_end
    set_param('pyle_tf/Pyle PLPW6D in Box LT/Inductor1','L','23.9');
    set_param('pyle_tf/Pyle Voltage-controlled/Inductor1','L','23.9');
    set_param('pyle_tf/Pyle Current-controlled/Inductor1','L','23.9');
    set_param('pyle_tf/Pyle Displacement-controlled/Inductor1','L','23.9');
    rad = 2*pi*i;
    disp(rad);
...
%     d_pid_ph = [d_pid_ph;i ((360 * out.d_pid_ph.signals.values(end)))];
    set_param('pyle_tf/Sine Wave','Frequency',num2str(rad));
    set_param('pyle_tf/Pyle PLPW6D in Box LT/Inductor1','L','28.68');
    set_param('pyle_tf/Pyle Voltage-controlled/Inductor1','L','28.68');
    set_param('pyle_tf/Pyle Current-controlled/Inductor1','L','28.68');
    set_param('pyle_tf/Pyle Displacement-controlled/Inductor1','L','28.68');
    out = sim('pyle_tf');
    lt_tf_err = [lt_tf_err;i 20*log10(max(out.lt_tf(:,2))/max(out.lt_tf(:,1)))];
    v_pid_tf_err = [v_pid_tf_err;i 20*log10(max(out.v_pid_tf(:,2))/max(out.v_pid_tf(:,1)))];
    i_pid_tf_err = [i_pid_tf_err;i 20*log10(max(out.i_pid_tf(:,2))/max(out.i_pid_tf(:,1)))];
    d_pid_tf_err = [d_pid_tf_err;i 20*log10(max(out.d_pid_tf(:,2))/max(out.d_pid_tf(:,1)))];
%     lt_delay_err = [lt_delay_err;i mean(dutycycle(out.lt_delay.signals.values(:,1),out.lt_delay.time,'Polarity','Positive'))*mean(pulseperiod(out.lt_delay.signals.values(:,1),out.lt_delay.time,'Polarity','Positive'))];
%     v_pid_delay_err = [v_pid_delay_err;i mean(dutycycle(out.v_pid_delay.signals.values(:,1),out.v_pid_delay.time,'Polarity','Positive'))*mean(pulseperiod(out.v_pid_delay.signals.values(:,1),out.v_pid_delay.time,'Polarity','Positive'))];
%     i_pid_delay_err = [i_pid_delay_err;i mean(dutycycle(out.i_pid_delay.signals.values(:,1),out.i_pid_delay.time,'Polarity','Positive'))*mean(pulseperiod(out.i_pid_delay.signals.values(:,1),out.i_pid_delay.time,'Polarity','Positive'))];
%     d_pid_delay_err = [d_pid_delay_err;i mean(dutycycle(out.d_pid_delay.signals.values(:,1),out.d_pid_delay.time,'Polarity','Positive'))*mean(pulseperiod(out.d_pid_delay.signals.values(:,1),out.d_pid_delay.time,'Polarity','Positive'))];
end
% %% Magnitude Plot
% mag_plot = figure;
...
%% Comparison Plot Magnitude
mag_comp_plot = figure;
plot(lt_tf(:,1),lt_tf(:,2));
hold on;
plot(v_pid_tf(:,1),v_pid_tf(:,2));
plot(i_pid_tf(:,1),i_pid_tf(:,2));
plot(d_pid_tf(:,1),d_pid_tf(:,2),'--');
plot(lt_tf_err(:,1),lt_tf_err(:,2));
plot(v_pid_tf_err(:,1),v_pid_tf_err(:,2));
plot(i_pid_tf_err(:,1),i_pid_tf_err(:,2));
plot(d_pid_tf_err(:,1),d_pid_tf_err(:,2));